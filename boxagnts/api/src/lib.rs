// boxagnts-api: Anthropic API client with streaming SSE support for Boxagnts
// Rust port.
//
// Handles:
...
use boxagnts_core::constants::{ANTHROPIC_API_VERSION, ANTHROPIC_BETA_HEADER};
use boxagnts_core::error::ClaudeError;
use boxagnts_core::types::{ContentBlock, Message, MessageContent, Role, ToolDefinition, UsageInfo};
...
pub mod client {
    ...
    impl Default for ClientConfig {
        fn default() -> Self {
            Self {
                api_key: String::new(),
                api_base: boxagnts_core::constants::ANTHROPIC_API_BASE.to_string(),
    ...
    impl AnthropicClient {
        ...
        fn apply_oauth_stealth(&self, request: &mut CreateMessageRequest) {
            ...
            let identity_block = SystemBlock {
                ...
                text: boxagnts_workspace::oauth_config::CLAUDE_CODE_SYSTEM_PROMPT_PREFIX.to_string(),
                cache_control: None,
            };
            ...
            request.system = match request.system.take() {
                ...
                Some(SystemPrompt::Blocks(mut blocks)) => {
                    ...
                    let already_first = blocks.first().is_some_and(|b| {
                        b.text == boxagnts_workspace::oauth_config::CLAUDE_CODE_SYSTEM_PROMPT_PREFIX
                    });
                    if !already_first {
        ...
        pub async fn from_config(cfg: &boxagnts_workspace::config::Config) -> anyhow::Result<Self> {
            let api_key = cfg
                .resolve_api_key().await
        ...
        pub async fn create_message(
            ...
            if self.config.api_key.is_empty() && self.config.provider != Provider::Codex {
                ...
                let hint = if model.starts_with("gemini") || model.starts_with("gemma") {
                    ...
                    "Set ANTHROPIC_API_KEY,  use --provider to select a different provider (e.g. --provider openai).".to_string()
                };
        ...
        pub async fn fetch_available_models(&self) -> anyhow::Result<Vec<crate::AvailableModel>> {
            ...
            if self.config.use_bearer_auth {
                let ua = format!(
                    "claude-cli/{}",
                    boxagnts_workspace::oauth_config::CLAUDE_CODE_VERSION_FOR_OAUTH
                );
                req = req
                    .header("anthropic-beta", boxagnts_workspace::oauth_config::OAUTH_BETA_FLAGS.join(","))
            ...
            loop {
                ...
                let anthropic_beta = if use_oauth {
                    let mut flags: Vec<&str> =
                        boxagnts_workspace::oauth_config::OAUTH_BETA_FLAGS.to_vec();
