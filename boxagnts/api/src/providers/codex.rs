...
use boxagnts_workspace::codex_oauth::{
    CODEX_API_ENDPOINT, CODEX_MODELS, CODEX_TOKEN_URL, DEFAULT_CODEX_MODEL,
};
use boxagnts_workspace::oauth_config::{get_codex_tokens, save_codex_tokens, CodexTokens};
use boxagnts_core::provider_id::{ModelId, ProviderId};
use boxagnts_core::types::UsageInfo;
...
impl CodexProvider {
    ...
    pub fn from_stored() -> Option<Self> {
        let tokens = futures::executor::block_on(async {  get_codex_tokens().await })?;
    ...
    async fn refresh_token(&self, refresh_token: &str) -> Result<String, ProviderError> {
        let body = json!({
            ...
            "client_id": boxagnts_workspace::codex_oauth::CODEX_CLIENT_ID,
        ...
        if let Err(e) = save_codex_tokens(&updated).await {
                                    ...
                                    match item.get("type").and_then(|value| value.as_str()) {
                                        ...
                                        Some("function_call") => {
                                            ...
                                            if open_blocks.insert(output_index) {
                                                yield Ok(StreamEvent::ContentBlockStart {
                                                    index: output_index,
                                                    content_block: boxagnts_core::types::ContentBlock::ToolUse {
                                                        id: call_id,
                                                        name,
                                                        input: json!({}),
                                                    },
                                ...
                                if let Some(part) = json_val.get("part") {
                                    if matches!(part.get("type").and_then(|value| value.as_str()), Some("output_text") | Some("text")) {
                                        ...
                                        if open_blocks.insert(output_index) {
                                            yield Ok(StreamEvent::ContentBlockStart {
                                                index: output_index,
                                                content_block: boxagnts_core::types::ContentBlock::Text {
                                                    text: String::new(),
                                                },
                                            });
                                        }
