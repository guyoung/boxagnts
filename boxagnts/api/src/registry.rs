...
fn provider_from_key(provider_id: &str, key: String) -> Option<Arc<dyn LlmProvider>> {
    ...
    match provider_id {
        ...
        "minimax" => Some(Arc::new(MinimaxProvider::new(None, key))),
...
pub fn provider_from_config(
    ...
    match provider_id {
        ...
        // coher
        "cohere" => api_key.map(|key| {
            let provider = CohereProvider::new(key);

            Arc::new(provider) as Arc<dyn LlmProvider>
        }),
        //copilot
        "github-copilot" => api_key.map(|key| {
            let provider = CopilotProvider::new(key);

            Arc::new(provider) as Arc<dyn LlmProvider>
        }),
        // google
        "google" => api_key.map(|key| {
            let mut provider = GoogleProvider::new(key);
        ...
        "minimax" => api_key.map(|key| {
            let provider = MinimaxProvider::new(api_base, key);
...
pub async fn runtime_provider_for(provider_id: &str) -> Option<Arc<dyn LlmProvider>> {
    use crate::providers::openai_compat_providers as p;
...
impl ProviderRegistry {
    ...
    /// `<cwd>/.boxagnts/codex_tokens.json`.  Returns `&mut self` for builder chaining.
    ...
    /// persistent [`AuthStore`] (`<cwd>/.boxagnts/auth.json`) for credentials.
    ///
    /// This ensures that API keys stored are picked up at startup,
    /// not just env vars.  Falls back to `from_environment` for
    /// providers that only support env-var config, and adds any
    /// extra providers that have keys in the auth store.
    ///
    /// [`AuthStore`]: boxagnts_core::AuthStore
    pub async fn from_environment_with_auth_store(anthropic_config: ClientConfig) -> Self {
    ...
    pub fn with_available_providers(&mut self) -> &mut Self {
            ...
            .unwrap_or(false)
        {
            ...
            self.register(Arc::new(MinimaxProvider::new(None, key)));
        }
