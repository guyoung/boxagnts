...
async fn token_path(server_name: &str) -> PathBuf {
    boxagnts_workspace::path::get_saved_dir().await
        .join("mcp-tokens")
        .join(format!("{}.json", server_name))
}

/// Persist an MCP OAuth token to disk.
pub async fn store_mcp_token(token: &McpToken) -> std::io::Result<()> {
    let path = token_path(&token.server_name).await;
...
pub async fn get_mcp_token(server_name: &str) -> Option<McpToken> {
    let path = token_path(server_name).await;
    let bytes = std::fs::read(&path).ok()?;
    serde_json::from_slice(&bytes).ok()
}

/// Delete the stored token for a server (effectively logs out).
pub async fn remove_mcp_token(server_name: &str) -> std::io::Result<()> {
    let path = token_path(server_name).await;
    if path.exists() {
...
fn build_mcp_auth_url(
    ...
    format!(
        "{}?client_id=boxagnts&redirect_uri={}&response_type=code&code_challenge={}&code_challenge_method=S256",
...
pub async fn run_mcp_auth_session(session: McpAuthSession) -> anyhow::Result<McpAuthResult> {
    ...
    store_mcp_token(&token).await.map_err(|e| {
        anyhow::anyhow!(
    ...
    let token_path = token_path(&token.server_name).await;
...
pub async fn get_valid_mcp_token(
    ...
    let Some(token) = get_mcp_token(server_name).await else {
        return Ok(None);
    };
...
pub async fn refresh_mcp_token(server_name: &str, token_endpoint: &str) -> anyhow::Result<McpToken> {
    let existing = get_mcp_token(server_name).await
    ...
    store_mcp_token(&new_token).await?;
    Ok(new_token)
}
...
mod tests {
    ...
    fn build_auth_url_contains_required_params() {
        ...
        assert!(url.contains("client_id=boxagnts"));
