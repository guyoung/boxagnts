...
// 当对话内容窗口已满（约 90% 以上）时，会自动删除较早的消息以腾出空间。
//
// Strategy:
...
use boxagnts_api::{AnthropicStreamEvent, ApiMessage, CreateMessageRequest, StreamAccumulator, StreamHandler, SystemPrompt};
use boxagnts_core::error::ClaudeError;
use boxagnts_core::types::{ContentBlock, Message, MessageContent, Role};
...
fn estimate_block_chars(block: &ContentBlock) -> usize {
    match block {
        ...
        ContentBlock::ToolResult { content, .. } => match content {
            boxagnts_core::types::ToolResultContent::Text(t) => t.len(),
            boxagnts_core::types::ToolResultContent::Blocks(blocks) => {
                ...
                match block {
                    ...
                    ContentBlock::ToolResult { tool_use_id, content, is_error } => {
                        let result_text = match content {
                            boxagnts_core::types::ToolResultContent::Text(t) => t.as_str().to_string(),
                            boxagnts_core::types::ToolResultContent::Blocks(_) => "[complex content]".to_string(),
                        };
...
pub struct CompactResult {
    ...
    pub messages: Vec<boxagnts_core::types::Message>,
...
pub fn snip_compact(messages: Vec<boxagnts_core::types::Message>, keep_n_newest: usize) -> (Vec<boxagnts_core::types::Message>, u64) {
    let total = messages.len();
...
pub fn calculate_messages_to_keep_index(messages: &[boxagnts_core::types::Message], token_budget: u64) -> usize {
...
/// `reactiveCompact.ts`.
fn strip_images(messages: Vec<boxagnts_core::types::Message>) -> Vec<boxagnts_core::types::Message> {
    use boxagnts_core::types::{ContentBlock, MessageContent};
...
/// `boxagnts_core::feature_gates::is_feature_enabled("reactive_compact")` is true.
///
/// The `cancel` token is checked before the API call so the user can abort
/// a long-running compact.
pub async fn reactive_compact(
    messages: Vec<boxagnts_core::types::Message>,
    client: &boxagnts_api::AnthropicClient,
    config: &crate::QueryConfig,
    cancel: tokio_util::sync::CancellationToken,
    recently_modified: &[std::path::PathBuf],
) -> Result<CompactResult, boxagnts_core::error::ClaudeError> {
    if cancel.is_cancelled() {
        return Err(boxagnts_core::error::ClaudeError::Cancelled);
    }
    ...
    for path in recently_modified.iter().take(MAX_FILES * 3) {
        ...
        new_messages.push(boxagnts_core::types::Message::user(text));
        injected += 1;
    }
...
pub async fn context_collapse(
    ...
    for msg in &messages {
        let role = match msg.role {
            boxagnts_core::types::Role::User => "Human",
            boxagnts_core::types::Role::Assistant => "Assistant",
        };
        let text = msg.get_all_text();
        if !text.is_empty() {
            transcript.push_str(&format!("{}: {}\n\n", role, text));
    ...
    if summary_text.is_empty() {
        return Err(boxagnts_core::error::ClaudeError::Other(
            "Context-collapse summary was empty".to_string(),
        ));
    }

    // Keep only: the synthetic summary + the most recent user turn.
    let collapse_notice = boxagnts_core::types::Message::user(format!(
...
pub fn collapse_read_tool_results(messages: Vec<boxagnts_core::types::Message>) -> Vec<boxagnts_core::types::Message> {
    use boxagnts_core::types::{ContentBlock, MessageContent, ToolResultContent};
...
pub fn collapse_search_results(messages: Vec<boxagnts_core::types::Message>) -> Vec<boxagnts_core::types::Message> {
    use boxagnts_core::types::{ContentBlock, MessageContent, ToolResultContent};
    ...
    let mut result: Vec<boxagnts_core::types::Message> = messages
        .into_iter()
        .rev()
        .map(|mut msg| {
            if let MessageContent::Blocks(ref mut blocks) = msg.content {
...
mod tests {
    use super::*;
    use boxagnts_core::types::{Message, Role};
