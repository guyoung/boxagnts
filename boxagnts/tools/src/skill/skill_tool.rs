...
//   <app>/extensions/skills/<name>/SKILL.md
//   <cwd>/.boxagnts/extensions/skills/<name>/SKILL.md
//
...
impl Tool for SkillTool {
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> ToolResult {
        ...
        let dirs = skill_search_dirs(ctx).await;
        ...
        let (_skill_path, raw) = match find_and_read_skill(&skill_name, &dirs).await {
            Some(found) => found,
            None => {
...
async fn skill_search_dirs(ctx: &ToolContext) -> Vec<PathBuf> {
    let mut dirs = vec![
        ctx.get_workspace_extensions_dir().await.join("skills")
    ];
    dirs.push(ctx.get_app_extensions_dir().await.join("skills"));
    dirs
}
...
async fn list_skills(dirs: &[PathBuf]) -> ToolResult {
                    ...
                    if path.is_dir() {
                        if let Some(dir_name) = path.file_name().and_then(|n| n.to_str()) {
                            ...
                            if skill_path.exists() && skill_path.is_file()
                                && !disk_skills.iter().any(|(n, _)| n == dir_name){
                                disk_skills.push((dir_name.to_string(), path));
                            }
                        }
    ...
    if total == 0 {
        return ToolResult::success(
            "No skills found. Create skill in <cwd>/extensions/skills/ to define skills.\n\
             Example: <cwd>/extensions/skills/review-code/SKILL.md"
                .to_string(),
        );
    }
