...
use crate::wasmtime_cli::common::{Profile, RunCommon, RunTarget};
use bytes::Bytes;
use clap::Parser;
...
impl RunCommand {
    ...
    #[cfg(feature = "run")]
    pub async fn execute(mut self) -> Result<(Bytes, Bytes)> {
        self.run.common.init_logging()?;

        let stdout_pipe = MemoryOutputPipe::new(1024 * 1024 * 10);
        let stderr_pipe = MemoryOutputPipe::new(1024 * 1024 * 10);

        let engine = self.new_engine()?;
        let main = self
            .run
            .load_module(&engine, self.module_and_args[0].as_ref())?;
        let (mut store, mut linker) =
            self.new_store_and_linker(&engine, &main, &stdout_pipe, &stderr_pipe)?;



        if let Err(err) = self.instantiate_and_run(&engine, &mut linker, &main, &mut store)
            .await {
            ...
            if !stderr.is_empty() {
                ...
                return  Ok((stdout, Bytes::from(err.to_string())))
            }
        }
    ...
    pub fn new_store_and_linker(
        ...
        self.populate_with_wasi(&mut linker, &mut store, &main, &stdout_pipe, &stderr_pipe)?;
    ...
    /// `wasi:cli2`-based commands and running their exported `run` function.
    #[cfg(feature = "component-model")]
                ...
                CliLinker::Core(linker) => {
                    match (self.run.common.wasi.preview2, self.run.common.wasi.threads) {
                        ...
                        (Some(false), _) | (None, Some(true)) => {
                            ...
                            self.set_legacy_p1_ctx(store, &stdout_pipe, &stderr_pipe)?;
                        }
                        ...
                        (Some(true), _) | (None, Some(false) | None) => {
                            ...
                            self.set_wasi_ctx(store, &stdout_pipe, &stderr_pipe)?;
                        }
                    }
                }
                #[cfg(feature = "component-model")]
    ...
    fn set_legacy_p1_ctx(
        &self,
        store: &mut Store<Host>,
        _stdout_pipe: &MemoryOutputPipe,
        _stderr_pipe: &MemoryOutputPipe,
    ) -> Result<()> {
