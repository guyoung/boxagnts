...
use wasmtime::{Engine, Result, Store, StoreContextMut, StoreLimits, UpdateDeadline, bail, error::Context as _, format_err};
...
pub struct ServeCommand {
    ...
    pub shutdown_addr: Option<SocketAddr>,
    ...
    #[arg(long)]
    pub max_instance_reuse_count: Option<usize>,
    ...
    #[arg(long)]
    pub max_instance_concurrent_reuse_count: Option<usize>,
...
impl ServeCommand {
    ...
    pub fn execute(mut self) -> Result<()> {
        ...
        // We force cli2 errors before starting to listen for connections so then
    ...
    fn add_to_linker(&self, linker: &mut Linker<Host>) -> Result<()> {
        ...
        if cli == Some(true) {
            ...
            #[cfg(feature = "component-model-async")]
            if self.run.common.wasi.p3.unwrap_or(crate::wasmtime_cli::common::P3_DEFAULT) {
