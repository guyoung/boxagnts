...
pub struct RunCommon {
    ...
    #[arg(long = "block_networks")]
    pub block_networks: Vec<String>,
    /*** ***/
}
...
impl RunCommon {
    ...
    pub fn load_module_contents(
        ...
        path: &Path,
        bytes: &[u8],
        _deserialize_module: impl FnOnce() -> Result<Module>,
        ...
        Ok(match Engine::detect_precompiled(bytes) {
            Some(Precompiled::Module) => {
                /*** ***/
                bail!("Core wasm module are not supported. You can use `wasm-tool` Convert a core wasm module into a component.");
    ...
    pub fn configure_wasip2(&self, builder: &mut WasiCtxBuilder) -> Result<()> {
        ...
        /*** ***/
        let allowed_outbound_hosts = crate::extension::net::parse_allowed_outbound_hosts(
        ...
        builder.socket_addr_check(move |addr, addr_use| {
            Box::pin({
                ...
                async move {
                    crate::extension::net::socket_addr_check(
                        addr,
                        addr_use,
        ...
        T: wasmtime_wasi::WasiView,
    {
        ...
        if self.common.wasi.p3.unwrap_or(P3_DEFAULT) {
            ...
            wasmtime_wasi::p3::add_to_linker_with_options(linker, &p3_options)
                .context("failed to link `wasi:cli2@0.3.x`")?;
        }
