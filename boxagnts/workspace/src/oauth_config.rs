...
// They will NOT work for Boxagnts — Anthropic's auth server will reject or misattribute requests.
// Users should use an API key from console.anthropic.com instead.
// To use OAuth, Boxagnts would need its own registered OAuth application with Anthropic.
pub const PROD_OAUTH: OAuthConfig = OAuthConfig {
    ...
    client_id: "9d1c250a-e61b-44d9-88ed-5944d1962f5e", // Anthropic's Claude Code — will not work for Boxagnts
    oauth_file_suffix: "",
...
pub const STAGING_OAUTH: OAuthConfig = OAuthConfig {
    ...
    client_id: "22422756-60c9-4084-8eb7-27705fd5cf9a", // Anthropic's Claude Code staging — will not work for Boxagnts
...
/// OpenAI Codex OAuth tokens, persisted to <cwd>/.boxagnts/codex_tokens.json
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
...
/// Path to the Codex tokens file (<cwd>/.boxagnts/codex_tokens.json)
async fn codex_tokens_path() -> Option<std::path::PathBuf> {
    let saved_dir = crate::path::get_saved_dir().await;

    Some(saved_dir.join("codex_tokens.json"))
}

/// Save Codex OAuth tokens to <cwd>/.boxagnts/codex_tokens.json
pub async fn save_codex_tokens(tokens: &CodexTokens) -> anyhow::Result<()> {
    let path = codex_tokens_path().await.ok_or_else(|| anyhow::anyhow!("Could not determine home directory"))?;
...
/// Load Codex OAuth tokens from <cwd>/.boxagnts/codex_tokens.json
pub async fn get_codex_tokens() -> Option<CodexTokens> {
    let path = codex_tokens_path().await?;
    if !path.exists() {
        return None;
    }
...
pub async fn clear_codex_tokens() -> anyhow::Result<()> {
    let path = codex_tokens_path().await.ok_or_else(|| anyhow::anyhow!("Could not determine home directory"))?;
    if path.exists() {
...
/// or by setting `BOXAGNTS_USE_OPENAI=1` with a manually stored token.
pub async fn is_codex_subscriber() -> bool {
    get_codex_tokens().await
