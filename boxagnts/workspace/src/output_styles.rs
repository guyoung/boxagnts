...
//!   - Global: `<app>/output-styles/`
//!   - Project: `<cwd>/.boxagnts/output-styles/`
//!
...
impl OutputStyleDef {
    ...
    pub fn builtin_default() -> Self {
        Self {
            ...
            description: "Standard Boxagnts responses.".to_string(),
            prompt: String::new(),
        }
    }
...
/// built-ins first, then styles from `<cwd>/.boxagnts/output-styles/`.
///

pub fn all_styles() -> Vec<OutputStyleDef> {
    let saved_dir = futures::executor::block_on(async {crate::path::get_saved_dir().await});

    let mut styles = builtin_styles();
    let user_dir = saved_dir.join("output-styles");
...
pub fn all_styles_with_runtime() -> Vec<OutputStyleDef> {
    let mut styles = all_styles();
